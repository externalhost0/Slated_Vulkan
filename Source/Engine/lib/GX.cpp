...
namespace Slate {
	...
	// if glfwWindow is nullptr than we run headless
										 ...
										 bool hasExternalMemory)
	{
		...
		if (hasExternalMemory) {
			...
			image_ci.pNext = &externalImageInfo;
	...
	void AllocatedBuffer::flushMappedMemory(const GX &ctx, VkDeviceSize offset, VkDeviceSize size) const {
		if (!isMapped()) {
			return;
		}
	...
	VkMemoryRequirements GX::getImageMemoryRequirements(VkImage image) const {
		...
		memoryRequirementsInfo2.pNext = nullptr;
		...
		vkGetImageMemoryRequirements2(_backend.getDevice(), &memoryRequirementsInfo2, &memoryRequirements2);
		return memoryRequirements2.memoryRequirements;
	}
	uint32_t GX::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlagBits memPropertyBits) const {
		VkPhysicalDeviceMemoryProperties memProps = {};
		vkGetPhysicalDeviceMemoryProperties(_backend.getPhysicalDevice(), &memProps);

		for (uint32_t i = 0; i < memProps.memoryTypeCount; ++i) {
			if ((typeFilter & (1 << i)) &&
				(memProps.memoryTypes[i].propertyFlags & memPropertyBits) == memPropertyBits) {
				return i;
			}
		}
		throw std::runtime_error("Failed to find suitable memory type.");
	}
	VmaAllocationInfo GX::getImageAllocInfo(InternalTextureHandle handle) {
		VmaAllocationInfo allocInfo;
		vmaGetAllocationInfo(_backend.getAllocator(), _texturePool.get(handle)->_vmaAllocation, &allocInfo);
		return allocInfo;
	}
