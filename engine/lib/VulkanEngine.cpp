...
//									   .set_required_features_13(features) //cant be on until 1.3 mvk
	...
	void VulkanEngine::CreateSwapchain() {
		...
		// COLOR
		{
			...
			_drawImage.imageExtent = tempDrawImageExtent;
			...
			VkImageCreateInfo rimg_info = vkinfo::CreateImageInfo(_drawImage.imageFormat, drawImageUsages, tempDrawImageExtent);
			...
			VkImageViewCreateInfo rview_info = vkinfo::CreateImageViewInfo(_drawImage.imageFormat, _drawImage.image, VK_IMAGE_ASPECT_COLOR_BIT);
			VK_CHECK(vkCreateImageView(_vkDevice, &rview_info, nullptr, &_drawImage.imageView));
		}
		// DEPTH
		{
			_depthImage.imageFormat = VK_FORMAT_D32_SFLOAT;
			_depthImage.imageExtent = tempDrawImageExtent;
			VkImageUsageFlags depthImageUsages = {};
			depthImageUsages |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;

			VkImageCreateInfo dimg_info = vkinfo::CreateImageInfo(_depthImage.imageFormat, depthImageUsages, tempDrawImageExtent);
			...
			//build a image-view for the draw image to use for rendering
			VkImageViewCreateInfo dview_info = vkinfo::CreateImageViewInfo(_depthImage.imageFormat, _depthImage.image, VK_IMAGE_ASPECT_DEPTH_BIT);
			VK_CHECK(vkCreateImageView(_vkDevice, &dview_info, nullptr, &_depthImage.imageView));
		}
	}
	...
	void VulkanEngine::ResizeSwapchain(GLFWwindow *pWindow) {
		{
			int w, h;
...
//				vmaDestroyAllocator(_allocator);
	...
	void VulkanEngine::DestroyBuffer(const vktypes::AllocatedBuffer& buffer) const {
		vmaDestroyBuffer(_allocator, buffer.buffer, buffer.allocation);
	}

	vktypes::GPUMeshBuffers VulkanEngine::UploadMesh(std::vector<Vertex> vertices) {
		...
		newSurface.vertexBufferAddress = vkGetBufferDeviceAddress(_vkDevice, &deviceAdressInfo);
	...
	vktypes::GPUMeshBuffers VulkanEngine::UploadMesh(std::span<Vertex> vertices, std::span<uint32_t> indices) {
	...
	vktypes::GPUMeshBuffers VulkanEngine::UploadMesh(std::vector<Vertex> vertices, std::vector<uint32_t> indices) {
		// just calculate sizes of our parameters
		const size_t vertexBufferSize = vertices.size() * sizeof(Vertex);
		const size_t indexBufferSize = indices.size() * sizeof(uint32_t);
